...
class Gadget2BinaryF77UnformattedType2Parser(object):
    ...
    def __init__(self, filename, verbose=True):
        ...
        if self.verbose: print "Parsing ICs generated by Toycluster 2.0\n"
    ...
    def read_ic(self, f):
        ...
        if self.verbose: print "First block", get_blockname(content)
        ...
        content = read_block(f)
        if self.verbose: print "Second block", get_blockname(content)
        ...
        content = read_block(f)
        if self.verbose: print "Third block", get_blockname(content)
        ...
        if self.Ngas > 0:
            ...
            if self.verbose: print "Fourth block", get_blockname(content)
            if get_blockname(content) == "U":
                content = read_block(f)
                self.u = numpy.frombuffer(content, dtype="float32", count=self.Ngas)
            content = read_block(f)

            if self.verbose: print "Fifth block", get_blockname(content)
            ...
            if self.verbose: print "Sixth block", get_blockname(content)[0:4]
